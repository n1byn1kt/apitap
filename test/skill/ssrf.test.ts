...
describe('SSRF validation', () => {
  describe('validateUrl', () => {
    ...
    it('rejects 0.0.0.0', () => {
      assert.equal(validateUrl('http://0.0.0.0/data').safe, false);
      assert.equal(validateUrl('http://0.0.0.0:8080/api').safe, false);
    });

    it('rejects IPv4-mapped IPv6 addresses (dotted-quad form)', () => {
      assert.equal(validateUrl('http://[::ffff:127.0.0.1]/data').safe, false);
      assert.equal(validateUrl('http://[::ffff:10.0.0.1]/data').safe, false);
      assert.equal(validateUrl('http://[::ffff:192.168.1.1]/data').safe, false);
      assert.equal(validateUrl('http://[::ffff:169.254.0.1]/data').safe, false);
    });

    it('rejects IPv4-mapped IPv6 addresses (hex form, Node-normalized)', () => {
      // Node normalizes [::ffff:127.0.0.1] to ::ffff:7f00:1
      assert.equal(validateUrl('http://[::ffff:7f00:1]/data').safe, false); // 127.0.0.1
      assert.equal(validateUrl('http://[::ffff:a00:1]/data').safe, false);  // 10.0.0.1
      assert.equal(validateUrl('http://[::ffff:c0a8:101]/data').safe, false); // 192.168.1.1
      assert.equal(validateUrl('http://[::ffff:a9fe:1]/data').safe, false);  // 169.254.0.1
    });

    it('rejects IPv6 link-local (fe80::/10)', () => {
      assert.equal(validateUrl('http://[fe80::1]/data').safe, false);
      assert.equal(validateUrl('http://[fe80::1%25eth0]/data').safe, false);
    });

    it('rejects IPv6 unique-local (fc00::/7)', () => {
      assert.equal(validateUrl('http://[fc00::1]/data').safe, false);
      assert.equal(validateUrl('http://[fd00::1]/data').safe, false);
      assert.equal(validateUrl('http://[fdab:1234::1]/data').safe, false);
    });
